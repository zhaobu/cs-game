...
func (d *desk) enterCalc() {
	...
	logrus.WithFields(logrus.Fields{"deskid": d.id}).Info("计算分数")
...
func (d *desk) breakGameVoteEnd() {
	...
	if voteEnd.Code == 1 {
		if d.currLoopCnt == 1 { // 第1局还没打完，就解散了，预扣的要还回去
			d.preSureCreater()
		}
...
func (d *desk) preSureCreater() {
	...
	_, err := mgo.UpdateWealthPreSure(d.createUserID, d.arg.FeeType, change)
	if err != nil {
...
func (d *desk) calc(landlordWin bool) {
	...
	for _, v := range d.sdPlayers {
		...
		if d.arg.Type == gameTypMatch {
			newV, err := mgo.UpdateWealth(v.uid, d.arg.FeeType, v.change)
			if err != nil {
				logrus.WithFields(logrus.Fields{"deskid": d.id, "err": err.Error()}).Error()
				continue
			}
	...
	if d.arg.Type == gameTypFriend && d.currLoopCnt == 1 {
		...
		if d.arg.PaymentType == 1 {
			...
			for _, v := range d.sdPlayers {
				newV, err := mgo.UpdateWealth(v.uid, d.arg.FeeType, int64(d.arg.Fee/seatNumber)*-1)
				if err != nil {
					continue
				}
				if d.arg.FeeType == 1 {
