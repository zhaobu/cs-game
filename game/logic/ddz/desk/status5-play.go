...
func (d *desk) enterPlay() {
	logrus.WithFields(logrus.Fields{"deskid": d.id}).Info("开始打牌")
	d.operNotif()
}
...
func (d *desk) systemPlay() {
	...
	f := logrus.Fields{"deskid": d.id, "uid": uid}
	if out != nil {
	...
	logrus.WithFields(f).Info("超时 系统出牌")
...
func (d *desk) handleUserOper(uid uint64, req *pbgame_ddz.UserOper) {
	...
	logrus.WithFields(logrus.Fields{
		"deskid": d.id, "status": d.f.Current(),
		"curruid": d.currPlayerID, "requid": uid,
		"oper": req.Oper.String(), "ct": req.Ct.String(),
		"out":     reqOutCard.String(),
		"carts":   fmt.Sprintf("%+v", req.Cards),
		"currseq": d.seq, "reqseq": req.Seq,
	}).Info("handleUserOper")
	...
	if req.Oper == pbgame_ddz.OperMask_OmNoout || req.Oper == pbgame_ddz.OperMask_OmPass {
		...
		req.Cards = nil
		if freeOut {
			logrus.WithFields(logrus.Fields{"deskid": d.id, "uid": uid}).Warn("need out card")
			d.toOne(&pbgame_ddz.ErrNotif{ErrType: 1}, uid)
			return
		}
...
func (d *desk) outCardCheck(uid uint64, req *pbgame_ddz.UserOper) bool {
	...
	if outCt == pbgame_ddz.CardType_CtUnknown || outCt != req.Ct {
		logrus.WithFields(logrus.Fields{"deskid": d.id, "uid": uid, "out": outC.String(), "outCt": outCt.String(), "reqCt": req.Ct.String()}).Warn("bad card type")
		return false
	}

	if !haveC.Have(req.Cards) {
		logrus.WithFields(logrus.Fields{"deskid": d.id, "uid": uid, "out": outC.String(), "have": haveC.String()}).Warn("not have card")
		return false
	}

	if !d.isFreeOut(uid) && !outC.Biger(d.lastGiveCard) {
		logrus.WithFields(logrus.Fields{"deskid": d.id, "uid": uid, "out": outC.String(), "last": d.lastGiveCard.String()}).Warn("not biger")
		return false
	}
