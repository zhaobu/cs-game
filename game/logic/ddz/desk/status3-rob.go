...
func (d *desk) enterRob() {
	logrus.WithFields(logrus.Fields{"deskid": d.id, "call uid": d.callUID}).Info("开始抢地主")

	f := logrus.Fields{"deskid": d.id}
	for _, v := range d.sdPlayers {
		f[fmt.Sprintf("%d", v.uid)] = v.currCard.String()
	}
	f["backcard"] = d.backCard.String()
	logrus.WithFields(f).Info("card info")
...
func (d *desk) robNotif() {
	...
	d.timer = tw.AfterFunc(timeOutCallrob*time.Second, func() {
		...
		if d.seq == seq {
			logrus.WithFields(logrus.Fields{"deskid": d.id, "uid": d.currPlayerID}).Info("抢地主超时")
...
func (d *desk) handleUserRob(uid uint64, req *pbgame_ddz.UserRob) {
	logrus.WithFields(logrus.Fields{"deskid": d.id, "status": d.f.Current(), "curruid": d.currPlayerID, "requid": uid, "code": req.Code}).Info("handleUserRob")
