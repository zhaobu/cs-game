...
func MakeDesk(uid uint64, makeDeskReq *pbgame.MakeDeskReq) {
	...
	defer func() {
		toGateNormal(log, makeDeskRsp, uid)
...
func JoinDesk(uid uint64, joinDeskReq *pbgame.JoinDeskReq) {
	...
	defer func() {
		toGateNormal(log, joinDeskRsp, uid)
...
func (d *desk) actionDestroy(uid uint64, req *pbgame.DestroyDeskReq) {
	...
	var uids []uint64
	defer func() {
		toGateNormal(d.loge, rsp, uid)
		if rsp.Code == 1 {
			toGateNormal(d.loge, &pbgame.DestroyDeskNotif{DeskID: d.id}, uids...)
			d.mu.Lock()
			d.preSureCreater()
			d.enterEnd()
			d.mu.Unlock()
...
func Action(uid uint64, actionName string, actionValue []byte) {
	...
	if d != nil {
		...
		log.Warnf("can not find desk by uid %d", uid)
	}
}
...
func (d *desk) action(uid uint64, actionName string, actionValue []byte) {
	...
	default:
		d.loge.Warnf("invalid type %s", actionName)
	}
}
