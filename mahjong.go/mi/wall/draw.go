...
// 获取下一张从前面抓的牌的索引
func (wall *Wall) getForwardNextIndex() int {
	var index int
...
func (wall *Wall) GetFrowrdNextTile() int {
	index := wall.getForwardNextIndex()
...
func (wall *Wall) ForwardDraw() int {
	defer func() {
		wall.SetExpectDisabled(wall.forward)
		wall.forward++
	}()
...
func (wall *Wall) ForwardDrawMulti(n int) []int {
	defer func() {
		for i := wall.forward; i < wall.forward+n; i++ {
			wall.SetExpectDisabled(i)
		}
		wall.forward += n
	}()
	tiles := make([]int, 0, n)
...
func (wall *Wall) getBackwordNextIndex() int {
	var index int
...
func (wall *Wall) BackwardDraw() int {
	...
	index := wall.getBackwordNextIndex()
	wall.SetExpectDisabled(index)
	return wall.GetTile(index)
}
