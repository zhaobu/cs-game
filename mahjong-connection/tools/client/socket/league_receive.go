...
func showRaceInfo(raceInfo *fbsCommon.Race) {
	...
	fmt.Println("raceInfo22:%+v", raceInfo)
	fmt.Println(fmt.Sprintf("raceInfo:%#v", raceInfo))
...
// 收到联赛大厅关闭的消息
func s2cCloseLeaguePush(impacket *protocal.ImPacket) {
	showClientDebug("[s2cCloseLeaguePush]received")
}
