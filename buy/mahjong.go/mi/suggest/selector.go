...
// GetRemainTilesCnt 读取给予的牌的剩余张数之和
func (ms *MSelector) GetRemainTilesCnt(tiles []int) int {
	return ms.getRemainTilesCnt(tiles)
}
...
// 判断某张牌是不是吊张
func (ms *MSelector) isDiaoTile(tile int) bool {
	// 单张超过1张，不算故障
	cnt, _ := ms.handTiles[tile]
	if cnt > 1 {
		return false
	...
	for _, rTile := range card.GetRelationTiles(tile) {
		...
		if _, exists := ms.handTiles[rTile]; exists && (rTile == tile-1 || rTile == tile+1) {
			isDiao = false
			break
		}
		if _, exists := ms.handTiles[rTile]; exists && (rTile == tile-2 || rTile == tile+2) {
			isDiao = true
			continue
		}
	}
...
// 获取指定牌中的孤张
func (ms *MSelector) getSpecifiedGuTiles(specified []int) []int {
	gTiles := make([]int, 0)
	for _, tile := range specified {
		if ms.isGuTile(tile) {
			gTiles = append(gTiles, tile)
		}
	}
...
// 获取指定牌中的吊张
func (ms *MSelector) getSpecifiedDiaoTiles(specified []int) []int {
	dTiles := make([]int, 0)
	for _, tile := range specified {
		if ms.isDiaoTile(tile) {
			dTiles = append(dTiles, tile)
		}
	}
	return dTiles
}
