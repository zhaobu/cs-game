...
// GetBackwoad 获取后游标
func (wall *Wall) GetBackward() int {
...
func (wall *Wall) Shuffle() {
	...
	wall.expect.initializtion(len(wall.tiles))
...
// 调换两个index的牌
func (w *Wall) exchange(from, to int) {
	w.tiles[from], w.tiles[to] = w.tiles[to], w.tiles[from]
}
